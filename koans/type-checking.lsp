...
(define-test test-guess-that-type!
    (let ((x (make-array '(5 3 9) :element-type 'string)))
      (assert-true (subtypep  (type-of x) '(SIMPLE-ARRAY T (* 3 *))))
      (assert-true (subtypep  (type-of x) '(SIMPLE-ARRAY T (5 * *))))
      (assert-true (subtypep  (type-of x) '(SIMPLE-ARRAY ARRAY *)))
      (assert-true (typep (make-array '(5 3 9) :element-type 'STRING ) (type-of x)))
;      (assert-true (typep (make-array '(5 3 33) :element-type
;      'VECTOR) (type-of x)))
      ))
